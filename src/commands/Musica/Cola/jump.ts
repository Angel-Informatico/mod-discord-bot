         ...
         return message.reply({
            embeds: [
               new ErrorEmbed().addField(
                  ...
                  client.translate(guildData.language, `${this.LANG_KEY}.error.invalid.value`, {prefix}),
               ),
            ],
         });
      indexOrigin -= 1;
         ...
         return message.reply({
            embeds: [
               new ErrorEmbed().addField(
                  ...
                  client.translate(guildData.language, `${this.LANG_KEY}.error.notfound.value`, {prefix}),
               ),
            ],
         });

      await player.queue.splice(0, indexOrigin);
      await player.skip();

      return message.reply({
         embeds: [
            new Embed().setDescription(
               client.translate(guildData.language, `${this.LANG_KEY}.success.description`, {name: originTrack.info.title}),
            ),
         ],
      });
   },
