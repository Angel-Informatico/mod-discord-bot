...
export default {
   async execute(client: Client, message, args, prefix, guildData) {
      ...
      const radioEntries = Object.entries(music.mixes);

      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const mixOptions = radioEntries.map(([key, value]) => {
         return {
            label: client.translate(guildData.language, `${this.LANG_KEY}.menu.options.${key}`, { name: key }),
            value: key,
            description: client.translate(guildData.language, `${this.LANG_KEY}.menu.options.playRadio`, { name: client.translate(guildData.language, `${this.LANG_KEY}.menu.options.${key}`, { name: key }) }),
         };
      });

      const rows:ActionRowBuilder<AnyComponentBuilder>[] = [];

      for (let i = 0; i < mixOptions.length; i += 25) {
         rows.push(
            new ActionRowBuilder()
            .addComponents(
               new StringSelectMenuBuilder()
                  .setCustomId(`MixSelection${i}`)
                  .setMaxValues(1)
                  .setMinValues(1)
                  .addOptions(
                     ...mixOptions.slice(i, i + 25)
                  )
            ),
         );
      }
      ...
      const totalRadios = mixOptions.length;
      for (let i = 0; i < rows.length; i += 4) {
         ...
         const embed = new Embed()
            ...
            .setFooter({
               text: client.translate(guildData.language, 'UTILS.MESSAGE.PAGINATION.message.embed.footer', {
                  page: currentPage,
                  pages: rows.length > 1 ? rows.length / 4 : rows.length,
               }),
         ...
         if(rows.length > 1) components.push(buttonRow)
      ...
      collector.on('collect', async (b) => {
         ...
         if (b?.isAnySelectMenu()) {
            ...
            const selectedRadioURL = music.mixes[fmName];
