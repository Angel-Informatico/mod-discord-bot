...
export default {
   ...
   ALIASES: ['mensual'],
   execute(client: Client, message, args: string[], prefix: string, guildData: IGuild, userData: IUser) {
      const amount = Math.floor(Math.random() * 250 + 50) * 30;
      const cooldown = 30 * 24 * 60 * 60 * 1000; // 1 month
